...
export default ({ onClick, visible })=> {
  ...
  useEffect(()=> {
    if(visible){
      ...
      gsap.to('#responsive-bar-top', .25, { y: 0, rotate: 0, backgroundColor: office.primaryColor });
      gsap.to('#responsive-bar-middle', .25, { opacity: 1, backgroundColor: office.primaryColor });
      gsap.to('#responsive-bar-bottom', .25, { y: 0, rotate: 0, backgroundColor: office.primaryColor });
    }
  },[visible])
